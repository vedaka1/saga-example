...
from user_service.presentation.http.filters import UserFiltersRequest
from user_service.presentation.http.requests import UserCreateRequest
from user_service.presentation.http.responses import UserResponse, UsersResponse
...
async def create_user(create_data: UserCreateRequest) -> UserResponse:
    ...
    async with client.start_session() as session:
        interactor = CreateUserInteractor(MongoUserRepository(database, CONFIG.mongodb, session=session))
...
async def get_user(user_id: str) -> UserResponse:
    client = init_mongo_db_client()
    database = client[CONFIG.mongodb.USER_DB_NAME]
    interactor = GetUserInteractor(MongoUserRepository(database, CONFIG.mongodb))
...
async def get_users(
    ...
    client = init_mongo_db_client()
    database = client[CONFIG.mongodb.USER_DB_NAME]
    interactor = GetUsersInteractor(MongoUserRepository(database, CONFIG.mongodb))
...
async def delete_user(user_id: str) -> None:
    client = init_mongo_db_client()
    database = client[CONFIG.mongodb.USER_DB_NAME]
    interactor = DeleteUserInteractor(MongoUserRepository(database, CONFIG.mongodb))
